...
| 类型 | 支持厂商 |
|---|---|
| **图片** | OpenAI、Gemini、MiniMax、火山引擎、阿里、Chatfire、Agnes AI |
| **视频** | MiniMax、火山引擎/Seedance、Vidu、阿里、Agnes AI |
...
- 适配 Agnes Video V2.0 API（新端点、新参数、新响应格式）
  - API 端点：`/v1/videos` + `/agnesapi?video_id=` 轮询
  - 参数：`num_frames`/`frame_rate`/`width`/`height`/`image`/`extra_body`
  - 完成状态：`completed`，视频 URL 在 `remixed_from_video_id` 字段
- 视频生成超时从 8 分钟延长至 30 分钟，匹配实际生成耗时
- 新增视频生成进度条（模拟估算）
- 新增视频删除与重新生成功能

#### 🎭 角色与配音增强

- TTS 语速调节：角色级（0.5x~2.0x）+ 镜头级独立调速
- Edge TTS 适配器：3 次重试 + 指数退避 + 空音频检测
- 角色形象描述（appearance）可编辑
- 移除已下线的 7 个中文音色，保留 6 个中文 + 3 个英文可用音色

#### 🖼️ 图片生成优化

- Agnes AI 图片适配器：修复 `response_format` 参数不兼容问题
- 候选图生成：后端生成多候选图，前端支持预览与勾选
- 镜头图片重新生成按钮
- 候选图点击放大预览 + 勾选图标选择

#### 🔧 工程改进

- 版本号自动跟随 Git commit hash（BUILD_HASH volume 映射）
- Docker 构建脚本（build.sh）集成 git hash + 自动部署
- 修复 Drizzle ORM schema 不匹配导致的刷新数据丢失问题
- 前端分镜编辑区 try-catch 容错保护
